...
import socket
import argparse
from scapy.all import Ether, IP, UDP, Raw, send#, raw#, raw, send
...
parser.add_argument('iface', type=str, help='interface name')
...
inner_udp = UDP(dport=UDP_PORT_NO,sport=1117)
inner_ip = IP(dst=UDP_IP_ADDRESS,src="127.0.0.1")
inner_headers = inner_ip/inner_udp/payload

send(inner_headers, iface=UDP_IF)
'''
payload = Raw(load="Today is a good day, you bloody wanker.")
